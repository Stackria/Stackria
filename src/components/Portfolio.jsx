    ...
    <section>
      <div>
        ...
        {portfolioList.map(({ title, details, link, img, icons: Icons, span, button }, index)=> (
          <div className="bg-slate-100 py-10 px-5 my-10" key={index}>
            <div >
              {/* Header */}
              <h1 className={`${museoReg.className}  text-slate-800 text-[18px] pb-5`}>{title}</h1>
              {/* Details */}
              <p className={`${outfitRegular.className} text-[15px]  text-slate-800 font-bold `}>{details}</p>
              {/* Tech Stack */}
              <div className="leading-10 mt-10">
                <h1 className={`${museoReg.className} text-slate-800 font-bold text-[14px]`}>{span}</h1>
                {/* Stack logo */}
                ...
                {/* Button */}
                <Link href={button.url}  className={`${styles.outfit} rounded-md bg-primary px-5 py-3 text-white text-[14px] shadow-lg shadow-blue-500/50 mt-5 font-bold hover:bg-secondary`}>{button.text}
                </Link>
              </div>
            </div>
            {/* Portfolio Image */}
